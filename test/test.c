...
static char* test_stack_push_pop() {
    WagerStack stack = {0};
...
static char* test_stack_overflow() {
    WagerStack stack = {0};
    stack.capacity = 2;
...
static char* test_init_player_hand() {
    Player player = {0};
    init_player_hand(&player);
    
    mu_assert("Hand size should be 5", player.hand_size == N_BETS_COLORS);
    mu_assert("Should have RED", player.hand[0] == BRED);
    mu_assert("Should have BLUE", player.hand[1] == BBLUE);
    mu_assert("Should have YELLOW", player.hand[2] == BYELLOW);
    mu_assert("Should have GREEN", player.hand[3] == BGREEN);
    mu_assert("Should have PURPLE", player.hand[4] == BPURPLE);
    
    return 0;
}

static char* test_has_card_in_hand() {
    Player player = {0};
    init_player_hand(&player);
...
static char* test_remove_all_cards() {
    ...
    mu_assert("Hand should be empty", player.hand_size == 0);
...
static char* test_players_initialized() {
    Game* game = setup_game();
    ...
    for (int i = 0; i < N_PLAYERS; i++) {
        ...
        mu_assert("Player should have full hand", game->players[i].hand_size == N_BETS_COLORS);
    }
    
    return 0;
}

static char* test_camels_on_board() {
    Game* game = setup_game();
...
static char* test_tickets_initialized() {
    Game* game = setup_game();
    
    for (int i = 0; i < N_BETS_COLORS; i++) {
        mu_assert("Should have 4 tickets per color", 
                  game->tickets[i].count == N_TICKETS);
        ...
        for (int j = 0; j < N_TICKETS; j++) {
            mu_assert("Tickets should start unowned", 
                      game->tickets[i].items[j].player_id == -1);
        }
    }
...
static char* test_assign_ticket() {
    Game* game = setup_game();
    
    int amount = assign_ticket(game, BRED, 0);
    mu_assert("Should get 5 point ticket first", amount == 5);
    mu_assert("Ticket should be assigned to player 0", 
              game->tickets[BRED].items[0].player_id == 0);
...
static char* test_assign_all_tickets() {
    Game* game = setup_game();
...
static char* test_place_spec_tile() {
    Game* game = setup_game();
...
static char* test_cant_place_spec_twice() {
    Game* game = setup_game();
...
static char* test_cant_place_spec_adjacent() {
    Game* game = setup_game();
...
static char* test_roll_dice() {
    ...
    mu_assert("Die value should be between -3 and 3", 
              die.value >= -3 && die.value <= 3 && die.value != 0);
    
    return 0;
}

static char* test_roll_all_dice() {
    Game* game = setup_game();
...
static char* test_move_camel_forward() {
    Game* game = setup_game();
...
static char* test_move_camel_to_finish() {
    Game* game = setup_game();
...
static char* test_get_top_camels() {
    Game* game = setup_game();
...
static char* test_assign_points_winner() {
    Game* game = setup_game();
    ...
    mu_assert("Winner ticket holder should get points", 
              game->players[0].points > initial_points);
    
    return 0;
}

static char* test_assign_points_second() {
    Game* game = setup_game();
    ...
    mu_assert("Second place ticket holder should get 1 point", 
              game->players[0].points == initial_points + 1);
    
    return 0;
}

static char* test_assign_points_loser() {
    Game* game = setup_game();
    ...
    mu_assert("Losing ticket holder should lose 1 point", 
              game->players[0].points == initial_points - 1);
...
static char* test_end_round_resets() {
    Game* game = setup_game();
    ...
    mu_assert("Player should be able to use spec again", 
              !game->players[0].used_spec);
    
    return 0;
}

static char* test_end_round_resets_hands() {
    Game* game = setup_game();
    
    // Remove some cards
    remove_card_from_hand(&game->players[0], BRED);
    remove_card_from_hand(&game->players[0], BBLUE);
    
    mu_assert("Hand should be reduced", game->players[0].hand_size == 3);
    
    end_round(game);
    
    mu_assert("Hand should be refilled", game->players[0].hand_size == N_BETS_COLORS);
...
static char* test_full_round_playthrough() {
    Game* game = setup_game();
...
static char* all_tests() {
    ...
    mu_run_test(test_end_round_resets_hands);
